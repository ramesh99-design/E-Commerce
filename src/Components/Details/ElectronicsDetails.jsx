...
export default function ElectronicsDetails({ product }) {
  const { id, name, price, stock, imageLink, description } = product;
  ...
  const currentStock = (stock !== undefined ? stock : 15);
  ...
  const handleAddToCart = () => {
    ...
    toast.success(`Added ${quantity}x "${name}" to cart! 🛍️`);
  };
  ...
  const originalPrice = (price * 1.22).toFixed(2);

  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 items-start">
      {/* Product Image Stage */}
      <div className="relative aspect-square w-full overflow-hidden rounded-3xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950/80 p-6 flex items-center justify-center shadow-md dark:shadow-2xl">
        <div className="absolute top-4 left-4 z-10">
          <span className="rounded-full border border-violet-500/30 bg-violet-500/15 dark:bg-violet-500/20 px-3 py-1 text-xs font-bold text-violet-700 dark:text-violet-300 backdrop-blur-md uppercase tracking-wider">
            Electronics
          </span>
        </div>
        ...
        <img
          ...
          alt={name}
          className="max-h-[85%] max-w-[85%] object-contain transition-transform duration-700 hover:scale-105 drop-shadow-xl"
        />
      </div>
        ...
        <div>
          ...
          <div className="mt-6 border-t border-slate-200 dark:border-slate-800/80 pt-6">
            ...
            <p className="mt-2 text-sm leading-relaxed text-slate-600 dark:text-slate-300">
              {description ||
                "Engineered with cutting-edge materials and state-of-the-art architecture. Delivers unrivaled efficiency, ultra-fast responsiveness, and sleek industrial durability."}
            </p>
          </div>
          ...
          <div className="mt-6 grid grid-cols-2 gap-3 text-xs">
            <div className="rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/50 p-3">
              ...
              <p className="mt-1 font-semibold text-slate-900 dark:text-white">High-Speed & Wireless</p>
            </div>
            <div className="rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/50 p-3">
              <span className="text-slate-500 dark:text-slate-400">Warranty</span>
              <p className="mt-1 font-semibold text-slate-900 dark:text-white">1 Year Official Brand</p>
            </div>
          </div>
        </div>
        ...
        <div className="mt-8 border-t border-slate-200 dark:border-slate-800/80 pt-6">
          ...
          <div className="mt-8 grid grid-cols-3 gap-3 border-t border-slate-200 dark:border-slate-800/80 pt-6 text-center">
            <div className="flex flex-col items-center gap-1">
              ...
              <span className="text-[10px] text-slate-500 dark:text-slate-400 font-medium">Free Express Delivery</span>
            </div>
            <div className="flex flex-col items-center gap-1">
              <RotateCcw className="h-4 w-4 text-sky-600 dark:text-sky-400" />
              <span className="text-[10px] text-slate-500 dark:text-slate-400 font-medium">30 Days Return</span>
            </div>
            <div className="flex flex-col items-center gap-1">
              <ShieldCheck className="h-4 w-4 text-emerald-600 dark:text-emerald-400" />
              <span className="text-[10px] text-slate-500 dark:text-slate-400 font-medium">100% Authentic</span>
            </div>
          </div>
        </div>
      </div>
